...
curl -X POST \
  ...
  -d '{ "name": "mqtt-src",
    "config":
    {
      ...
      "producer.override.partitioner.class": "de.paul.kafka.connect.SparkplugEdgeNodeIdPartitioner"
...
curl -X POST \
  ...
  -d '{ "name": "mqtt-sink",
    "config":
    {
      ...
      "transforms.topicTransform.type": "io.confluent.connect.transforms.ExtractTopic$Key",
